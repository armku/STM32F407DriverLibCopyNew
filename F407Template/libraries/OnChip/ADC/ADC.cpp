...
u8 MADC::GetBattle(u8 chNum)
{
	static float voltage = 0, power = 0;
	voltage = voltage + 0.01 * ( _adcValue[_chMap[chNum]] * 0.0008056640625 * (3) - voltage);
	power =  (627041.324*pow(voltage,4) - 18023682.682*pow(voltage,3) + 194049355.407*pow(voltage,2) - 927329499.351*voltage + 1659531143.78290)/99889.066;
	if(voltage>7.6)
		power=1.00;
	if(voltage<6.67)
		power=0.00;
