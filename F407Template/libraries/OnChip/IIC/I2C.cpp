...
I2C::I2C(I2C_TypeDef* i2c, u32 speed, u8 remap, u8 priGroup, u8 preEvt, u8 subEvt, u8 preErr, u8 subErr)
{
	...
	if (mI2C == I2C1)         //I2C1
	{
		...
		{		                            //		        | I2C1 |-------------|-------------|             |RCC_APB1Periph_I2C1 |
			mSCLPinSource = GPIO_PinSource6;
			mSDAPinSource = GPIO_PinSource7;
		...
		mGPIOAF = GPIO_AF_I2C1;
		mSCLPort = GPIOB;
		mSDAPort = GPIOB;
		mSCLGPIORcc = RCC_AHB1Periph_GPIOB;
		mSDAGPIORcc = RCC_AHB1Periph_GPIOB;
#ifdef USE_I2C1
		pI2C1 = this;
#endif
	}
	...
	{
		mSCLPinSource = GPIO_PinSource10;
		mSDAPinSource = GPIO_PinSource11;
	...
	else if (mI2C == I2C3)               //I2C3
	{
		mSCLPinSource = GPIO_PinSource8;
		mSDAPinSource = GPIO_PinSource9;
		mSCLPin = GPIO_Pin_8;	          //SCL Pin	
		mSDAPin = GPIO_Pin_9;           //SDA Pin 	
		mEvtIRQ = I2C3_EV_IRQn;         //Event IRQ
		mErrIRQ = I2C3_ER_IRQn;         //Error IRQ
		mI2CRcc = RCC_APB1Periph_I2C3;  //I2C Clock
		mGPIOAF = GPIO_AF_I2C3;
		mSCLPort = GPIOA;
		mSDAPort = GPIOC;
		mSCLGPIORcc = RCC_AHB1Periph_GPIOA;
		mSDAGPIORcc = RCC_AHB1Periph_GPIOC;
#ifdef USE_I2C3
		pI2C3 = this;
#endif
	}
...
{
	RCC_APB2PeriphClockCmd(mSDAGPIORcc, ENABLE);     //GPIO Clock
	RCC_APB2PeriphClockCmd(mSCLGPIORcc, ENABLE);     //GPIO Clock
	RCC_APB1PeriphClockCmd(mI2CRcc, ENABLE);
	
	GPIO_PinAFConfig(mSCLPort, mSCLPinSource, mGPIOAF); //¸´ÓÃ
	GPIO_PinAFConfig(mSDAPort, mSDAPinSource, mGPIOAF); //¸´ÓÃ
